...
export default function CartModal({ product, player, onClose }) {
  ...
  const applyRedeem = async () => {
    ...
    try {
      const res  = await fetch('/api/orders/apply-code',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({code:redeemInput.trim(),productId:product.id,price:basePrice})});
  ...
  const handleCheckout = async () => {
    setLoading(true);
    try {
      const res  = await fetch('/api/orders/create',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'include',body:JSON.stringify({productId:product.id,redeemCode:redeemInfo?.code||null})});
      const data = await res.json();
                ...
                <div style={{display:'flex',gap:8}}>
                  <div style={{position:'relative',flex:1}}>
                    ...
                    <input value={redeemInput} onChange={e=>setRedeemInput(e.target.value.toUpperCase())}
                      ...
                      style={{paddingLeft:32,fontFamily:'monospace',fontSize:13}} disabled={!!redeemInfo}/>
                  </div>
                  {redeemInfo
