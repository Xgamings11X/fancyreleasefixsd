...
export default function StorePage({ settings, categories, products }) {
  ...
  const allTabs = [
    { id:'all', label:'Semua', icon:'fa-store' },
    ...categories.map(c=>({id:c.slug, label:c.name, icon:null, emoji:c.icon})),
  ];
    ...
    <>
      ...
      <div style={{padding:'130px 6% 80px',maxWidth:1200,margin:'0 auto'}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:32,flexWrap:'wrap'}}>
          <div className="tabs-container" style={{flex:1,overflowX:'auto',minWidth:0}}>
            {allTabs.map(tab=>(
          ...
          <div style={{position:'relative',flexShrink:0}}>
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)}
              ...
              style={{paddingLeft:36,width:200,borderRadius:10}}/>
          </div>
        </div>
