...
export default function HomePage({ settings }) {
  ...
  const socials = [
    ...
    s.whatsapp_url&& { href:s.whatsapp_url,cls:'btn-wa',      icon:'fa-whatsapp',  label:'Whatsapp',brand:true },
    s.tiktok_url  && { href:s.tiktok_url,  cls:'btn-tiktok',  icon:'fa-tiktok',    label:'Tiktok',  brand:true },
    s.youtube_url && { href:s.youtube_url, cls:'btn-ig',      icon:'fa-youtube',   label:'YouTube', brand:true },
  ].filter(Boolean);
  ...
  return (
    <>
      ...
      <header style={{minHeight:'100vh',display:'flex',flexDirection:'column',justifyContent:'center',alignItems:'center',textAlign:'center',padding:'160px 24px 60px'}}>
        ...
        {s.logo_url
          ? <img src={s.logo_url} alt={serverName} style={{maxWidth:180,marginBottom:24,filter:'drop-shadow(0 10px 25px rgba(255,107,0,0.15))'}}/>
